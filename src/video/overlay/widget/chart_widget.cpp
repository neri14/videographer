...
{
    log.debug("Created {}/{} chart widget", y_key, x_key);
}
...
{
    log.debug("Preloading {}/{} chart", y_key_, x_key_);
    ...
    if (min_x < max_x && min_y < max_y) {
        ...
        log.warning("Not enough data points containing both '{}' and '{}' for charting", x_key_, y_key_);
    }
}
...
{
    if (!valid) {
        log.warning("Invalid widget ignoring");
        return;
    }
